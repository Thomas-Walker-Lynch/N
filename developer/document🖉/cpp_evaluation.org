...
1. Macro function definition
   ...
   Macro function definitions are not evaluated, so the body (the definition) of the macro remains literal until the time it is called.  Thus macro calls in macro defintion remain
   literal, unevaluated.
...
2. directives
   ...
   #ifdef and #ifndef to not evaluate what follows. They expect a full formed
   macro name.
