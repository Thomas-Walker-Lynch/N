...
#define TM·FACE
  ...
  #ifndef CVT
    typedef struct Ξ(TM·Array ,CVT);

    // tape and area are included with Tape Machine to facilitate abstract interfaces.
    typedef struct{

      struct {
        Core·Status      (*topo)  ( Ξ(TM·Array ,CVT) *tm ,TM·Tape·Topo *result );
        Ξ(extent_t ,CVT) (*extent)( Ξ(TM·Array ,CVT) *tm );
      } tape;

      struct {
        // Initialize tm
        Core·Status (*mount_pe)(Ξ(TM·Array ,CVT) *tm ,CVT position[] ,Ξ(extent_t ,CVT) extent);
        Core·Status (*mount_pp)(Ξ(TM·Array ,CVT) *tm ,CVT *position_left ,CVT *position_right);
      } area;

      // tape machine functions
      Core·Status (*mount)   (Ξ(TM·Array ,CVT) *tm);
      Core·Status (*dismount)(Ξ(TM·Array ,CVT) *tm);

      TM·Head·Status (*status)        (Ξ(TM·Array ,CVT) *tm ,TM·Head·Status *status);
      Core·Status    (*head_on_format)(Ξ(TM·Array ,CVT) *tm ,bool *flag);

      bool (*can_read)    (Ξ(TM·Array ,CVT) *tm);
      bool (*on_origin)   (Ξ(TM·Array ,CVT) *tm);
      bool (*on_rightmost)(Ξ(TM·Array ,CVT) *tm);

      void (*step)      (Ξ(TM·Array ,CVT) *tm);
      void (*step_left) (Ξ(TM·Array ,CVT) *tm);
      void (*step_right)(Ξ(TM·Array ,CVT) *tm); // Synonym for step
      void (*rewind)    (Ξ(TM·Array ,CVT) *tm);

      void (*copy_datum)( Ξ(TM·Array ,CVT) *tm_read ,Ξ(TM·Array ,CVT) *tm_write );
      void (*read)      ( Ξ(TM·Array ,CVT) *tm ,CVT *remote_pt );
      void (*write)     ( Ξ(TM·Array ,CVT) *tm ,CVT *remote_pt );

    } Ξ(TM·Array ,CVT)·FG;

    Ξ(TM·Array ,CVT)·FG Ξ(TM·Array ,CVT)·fg;
